    ...
    <div className="lg:flex lg:h-full lg:min-h-0 lg:flex-col">
      ...
      >
        {orderedModels.map((model, index) => {
          ...
          // Surface input modalities the model explicitly rejects. Absent
          // fields mean "supported" (the default), so we only call out the
          // disabled ones to keep the tooltip quiet for normal models.
          if (caps?.inputImages === false) {
            capLabels.push("No images");
          }
          if (caps?.inputAudio === false) {
            capLabels.push("No audio");
          }
          if (caps?.inputFiles === false) {
            capLabels.push("No files");
          }
          const contextLabel = formatModelMetadataTokens(model.contextWindowTokens);
          if (contextLabel) capLabels.push(`${contextLabel} context`);
          if (model.pricing) {
            ...
            if (inputRate && outputRate) capLabels.push(`${inputRate} in · ${outputRate} out / 1M`);
          }
