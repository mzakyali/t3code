    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* Effect.gen(function* () {
          ...
          // Sources are keyed by canonical path: /var → /private/var on macOS.
          assert.include(
  ...
  it.live("does not share an in-flight scan after custom prices change", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        let homeProbes = 0;
        // Sources are keyed by canonical path: /var → /private/var on macOS.
        const claudeProjects = NodePath.join(home, "claude", "projects");
        const canonicalClaudeProjects = yield* Effect.promise(() =>
          NodeFSP.realpath(claudeProjects).catch(() => claudeProjects),
        );
        const service = yield* UsageService.make.pipe(
          Effect.provideService(FileSystem.FileSystem, {
            ...
            exists: (path) =>
              fileSystem.exists(path).pipe(
                Effect.tap(() => {
                  if (path !== claudeProjects && path !== canonicalClaudeProjects)
                    return Effect.void;
                  homeProbes += 1;
