  ...
  const resolveTranscriptDirs = Effect.fn("UsageService.resolveTranscriptDirs")(function* (
    ...
    for (const driver of ["claudeAgent", "codex", "grok"] as const) {
      ...
      for (const instance of instances) {
        ...
        const volumeId = stableVolumeId(sourceKey, provider, dir, currentVolumeId);
    ...
    // Devin's usage comes from this server's canonical ACP event logs rather
    // than a CLI transcript directory, so it sits outside the instance loop.
    const devinDir = config.providerLogsDir;
    const devinVolumeId = stableVolumeId(
      `devin\0${devinDir}`,
      "devin",
      devinDir,
      yield* Effect.promise(() => readDirectoryVolumeId(devinDir)),
    );
    dirs.push({
      provider: "devin",
      dir: devinDir,
      volumeId: devinVolumeId,
      eventLog: true,
    });
    return dirs;
  });
